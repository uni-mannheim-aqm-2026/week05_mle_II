...
author: 
  - Domantas Undzėnas, Lisa-Marie Müller
...
In the lecture we have seen that the **Fisher Information Matrix** plays a crucial role when we want to calculate MLE standard errors and subsequently apply these to test hypotheses. The reason is that this matrix closely corresponds to the asymptotic variance-covariance matrix. The Fisher Information Matrix is defined as:
...
Let's first have a look at the code we used last week to optimize the log-likelihood of the exponential model:
